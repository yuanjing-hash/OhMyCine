...
assert.match(playerView, /function handleWindowBlur\(\) \{\s+controlsInteracting\.value = false\s+playerControlsRef\.value\?\.dismissTransientUi\(\)\s+void releaseHeldArrow\(false\)\s+scheduleChromeHide\(\)/)
assert.match(playerView, /function handleApplicationPointerLeave\(\)[\s\S]*controlsInteracting\.value = false[\s\S]*dismissTransientUi\(\)[\s\S]*scheduleChromeHide\(\)/)
