...
assert.match(mobileNavigation, /我的收藏/)
assert.match(mobileNavigation, /route\.name === 'favorites'/)
assert.match(mobileNavigation, /grid-template-columns: repeat\(5, minmax\(0, 1fr\)\);/)
...
assert.match(playerView, /<MobilePlayerControls[\s\S]*?@open-playback-detail="openPlaybackDetailFromContextMenu"/)
assert.match(playerView, /<MobilePlayerControls[\s\S]*?@navigate-home="navigateFromContextMenu\('home'\)"/)
assert.match(playerView, /<MobilePlayerControls[\s\S]*?@navigate-settings="navigateFromContextMenu\('settings'\)"/)
assert.match(playerView, /'player-detail-panel--native-mobile': isNativeAndroidPlayer/)
assert.match(playerView, /\.player-detail-panel\.player-detail-panel--native-mobile \{[\s\S]*?top: max\(0\.75rem, env\(safe-area-inset-top\)\);[\s\S]*?left: max\(0\.75rem, env\(safe-area-inset-left\)\);/)
