...
watch(() => props.modelValue, (value) => {
  if (value)
    revealedValue.value = ''
  else if (!revealedValue.value)
    revealed.value = false
})
watch(() => props.resetKey, clearReveal)
watch(() => props.configured, (configured) => {
  if (!configured && !props.modelValue)
    clearReveal()
})
...
async function toggleReveal() {
  ...
  if (!props.configured || !props.loadSecret)
    return
  ...
  try {
    ...
    if (generation !== revealGeneration)
      return
    if (!value)
      throw new Error('该凭据尚未配置')
    revealedValue.value = value
    revealed.value = true
  }
  catch (reason) {
    if (generation === revealGeneration)
      emit('revealError', reason instanceof Error ? reason.message : '读取已保存凭据失败')
  }
  finally {
    if (generation === revealGeneration)
      revealing.value = false
...
<template>
  <div class="relative w-full">
    ...
    <button
      type="button"
      class="absolute right-2 top-2 flex h-8 w-8 items-center justify-center rounded-xl text-white/42 transition-colors hover:bg-white/10 hover:text-white/78 disabled:cursor-not-allowed disabled:opacity-35"
