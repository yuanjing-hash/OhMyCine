...
type WebDavCredentialReader = (ref: string) => Promise<WebDavCredentialValue | null>

export type WebDavFetch = <T = unknown>(
  request: Parameters<typeof ofetch>[0],
  options?: Parameters<typeof ofetch>[1],
) => Promise<T>

interface WebDavConfigExtra {
  readonly credentialRef?: string
  readonly rootPath: string
}

interface WebDavFileRecord {
...
export interface WebDavDataSourceOptions {
  readonly fetcher?: WebDavFetch
  readonly readCredential?: WebDavCredentialReader
}

export interface WebDavLoginConfigInput {
  readonly id: string
  readonly url: string
...
export interface WebDavLoginConfigResult {
  readonly config: DataSourceConfig
  readonly libraries: MediaLibrary[]
}

export interface WebDavSetupSessionInput {
  readonly id: string
  readonly url: string
  readonly displayName?: string
  readonly username: string
...
export class WebDavDataSource implements DataSource {
  private config: DataSourceConfig | null = null
  private baseUrl = ''
  private credentialRef = ''
  private credential: WebDavCredentialValue | null = null
  private rootPath = '/'
  private connected = false
  private readonly cache = new SourceMetadataCache()
  private readonly fetcher: WebDavFetch
  private readonly readCredential: WebDavCredentialReader

  readonly type = 'webdav' as const

  constructor(options: WebDavDataSourceOptions = {}) {
    this.fetcher = options.fetcher ?? (ofetch as WebDavFetch)
    this.readCredential = options.readCredential ?? readWebDavCredential
  }
  ...
  get name(): string {
    return this.config?.displayName ?? this.config?.name ?? 'WebDAV'
  }
  ...
  async init(config: DataSourceConfig): Promise<void> {
    ...
    this.baseUrl = normalizeWebDavBaseUrl(config.url)
    const extra = readWebDavExtra(config)
  ...
  private async resolvePlayablePath(id: string): Promise<string> {
    if (isRawScannedSyntheticId(id))
      throw new Error('WebDAV 剧集合集不能直接播放，请选择具体分集。')
    ...
    this.ensureRecordInRoot(record)
    if (record.isDir)
      throw new Error('WebDAV 文件夹不能直接播放。')
    if (!isVideoFileName(record.name))
      throw new Error('该 WebDAV 文件不是支持的视频格式。')

    return path
  }

  private async requestPropfind(path: string, depth: 0 | 1): Promise<WebDavFileRecord[]> {
    this.ensureConfigured()
    const providerPath = this.resolveLibraryPath(path)
    try {
  ...
  private async ensureCredential(): Promise<WebDavCredentialValue> {
    this.ensureConfigured({ requireCredential: false })
    if (this.credential)
    ...
    if (!credential)
      throw new Error('WebDAV 登录凭证缺失。请在设置的数据源管理中重新编辑并登录。')
  ...
  private async readStoredCredential(): Promise<WebDavCredentialValue | null> {
    if (!this.credentialRef)
      return null
    return this.readCredential(this.credentialRef)
  }

  private mapItem(record: WebDavFileRecord): MediaItem {
    return {
      id: record.path,
      sourceId: this.id,
  ...
  private mapMediaSource(record: WebDavFileRecord): MediaSourceOption {
    return {
      id: 'default',
      name: 'WebDAV',
  ...
  private buildWebDavUrl(path: string, directory: boolean): string {
    if (!isPathWithinRoot(path, this.rootPath))
      throw new Error('WebDAV 路径不在已选择的根目录内。')
  ...
  private resolveLibraryPath(path?: string): string {
    ...
    const normalized = normalizeWebDavPath(raw)
...
export async function createAuthenticatedWebDavSetupSource(input: WebDavSetupSessionInput): Promise<WebDavDataSource> {
  const credential = normalizeLoginCredential(input.username, input.password)
  const displayName = input.displayName?.trim() || 'WebDAV'
  const credentialRef = createCredentialRef(input.id, 'webdav')
...
export async function loginWebDavAndCreateConfig(input: WebDavLoginConfigInput): Promise<WebDavLoginConfigResult> {
  const credential = normalizeLoginCredential(input.username, input.password)
  const credentialRef = createCredentialRef(input.id, 'webdav')
  const displayName = input.displayName?.trim() || 'WebDAV'
  ...
  try {
    await saveWebDavCredential(credentialRef, credential)
  }
  catch (error) {
    throw new Error(redactSensitiveText(error))
  }
...
export function readWebDavRootPath(config: DataSourceConfig | null | undefined): string {
  if (!config || config.type !== 'webdav')
    return '/'
  try {
    return readWebDavExtra(config).rootPath
  }
  catch {
    return '/'
  }
...
export function normalizeWebDavRootPath(value: string | undefined): string {
  return normalizeWebDavPath(value)
}

function normalizeLoginCredential(username: string, password: string): WebDavCredentialValue {
  const trimmedUsername = username.trim()
  if (!trimmedUsername || !password)
    throw new Error('请输入 WebDAV 账号和密码。')
  return {
...
function normalizeWebDavBaseUrl(value: string): string {
  ...
  if (!trimmed)
    throw new Error('请输入 WebDAV URL。')

  try {
    const url = new URL(trimmed)
    if (url.protocol !== 'http:' && url.protocol !== 'https:')
      throw new Error('WebDAV URL 仅支持 http 或 https。')
    if (url.username || url.password || url.search || url.hash)
      throw new Error('WebDAV URL 不能包含账号、密码、查询参数或片段。')

    const pathname = url.pathname.replace(/\/+$/, '')
    return `${url.origin}${pathname === '/' ? '' : pathname}`
  }
  catch (error) {
    if (error instanceof Error && error.message.includes('WebDAV'))
      throw error
    throw new Error('WebDAV URL 格式无效。')
  }
}

function readWebDavExtra(config: DataSourceConfig): WebDavConfigExtra {
  if (config.type !== 'webdav')
    throw new Error('WebDAV 数据源类型无效。')

  const extra = config.extra ?? {}
  return {
    credentialRef: typeof extra.credentialRef === 'string' ? extra.credentialRef : undefined,
    rootPath: typeof extra.rootPath === 'string' ? normalizeWebDavPath(extra.rootPath) : '/',
  }
}

function normalizeWebDavPath(value: string | undefined): string {
...
function sanitizeExportConfig(config: DataSourceConfig | null): DataSourceConfig {
  ...
  return {
    ...config,
    url: normalizeWebDavBaseUrl(config.url),
    extra: safeExtra,
  }
}
...
function parsePropfindRecords(xml: string, baseUrl: string): WebDavFileRecord[] {
  return matchXmlBlocks(xml, 'response')
    .filter(isSuccessfulResponseBlock)
    .map((block): WebDavFileRecord | null => {
    ...
    .filter((record): record is WebDavFileRecord => record != null)
}
...
function createFallbackRecord(path: string): WebDavFileRecord {
  return {
