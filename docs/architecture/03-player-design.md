...
单视频偏好恢复分为两段：倍速、字幕偏移和画面模式可立即恢复；字幕、音轨和缓存外部字幕必须等待时长/轨道元数据出现后再匹配。`sid`、`aid` 与 `sub-add` 使用 libmpv 异步 API 入队，Rust 立即释放 `MpvState` 锁，并由事件转发器清理异步回复，禁止在远程流初始化阶段用同步轨道命令阻塞时间采集和全部播放器控制。
