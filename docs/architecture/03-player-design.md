...
播放器控制与页面导航使用两套普通全局快捷键映射。播放器映射仅在播放页生效，`H` 默认立即隐藏/恢复控制 UI；控制栏按钮按稳定动作顺序默认映射为 `QWERTYUIOP[]`：上一集、后退 10 秒、播放/暂停、前进 10 秒、下一集、静音、倍速、字幕、音轨、队列、播放设置、全屏。音轨或队列按钮不可用时对应按键无操作，后续动作不得因条件按钮隐藏而串位。播放器页发生同键冲突时播放动作优先，其他页面仍可执行导航动作。
