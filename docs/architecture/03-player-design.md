...
没有触摸硬件时，按住 `Alt` 再使用播放画面的鼠标主键，可把当前操作映射到同一手势状态机。Alt 模拟只接管无控件遮挡的播放画面，并在实际操作期间暂停鼠标移动自动唤出控制 UI，以便验证触摸单击显隐；控制栏、菜单、输入框、链接和右键菜单始终使用桌面鼠标逻辑。松开并结束当前手势后立即恢复普通鼠标行为，期间显示轻量测试状态标记。
...
这些工作只代表手机 Web UI 与通用触屏交互可评审，不代表 Android 已可发布。Android 版本仍需独立完成原生 libmpv 渲染、媒体与目录权限、系统返回键、系统栏避让和移动端更新策略；桌面窗口拖拽、最小化、最大化与关闭按钮在移动环境中不得作为导航依赖。
