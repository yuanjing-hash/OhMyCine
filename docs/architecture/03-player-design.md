...
- 聚合首页和单数据源根页使用进程内会话快照：首次成功加载后，返回页面立即显示最近一次内容，5 分钟内不重复请求；过期时保留旧内容并后台刷新。快照只保存媒体库和展示元数据，不持久化凭据、认证 Header、原始播放 URL 或签名 URL；修改/删除数据源、手动清理缓存和明确播放进度刷新会精确失效对应快照。
