...
TMDB 凭据是可选增强，不是扫描可用性的前置条件。未配置用户 token/key、TMDB 超时或请求失败时，扫描应继续保留可播放候选、目录识别、文件名解析和季集结构；这些缺失匹配结果或 `notConfigured` / `notFound` / `failed` / `skipped` 等未完成匹配条目在媒体库分类卡片中统一进入 `未识别`，不再因为解析出 movie、tv 或 path category hint 而拆成多个看似已分类的卡片。后续可以接入内置/公共元数据通道，但客户端不得硬编码第三方秘密 key。
...
手机播放器控制栏按三层重排：时间与可触摸拖动进度、主播放控制、字幕/音轨/队列/画面/全屏工具。轨道、倍速、队列和画面设置在窄屏下使用底部面板尺寸，进度条使用 Pointer Events 和 pointer capture，同一实现兼容鼠标与触摸拖动。普通浏览器预览不得因无 Tauri 窗口对象而中断 Vue 挂载，以便固定使用 `390×844` 等设备尺寸进行响应式截图回归。
...
当前已生成 Tauri Android Studio 工程，并可通过 `npm run tauri:build:android:preview` 构建 ARM64 debug APK。Activity 显式启用 `resizeableActivity` 且不锁定方向，可用于真机评审手机/平板 UI、横竖屏和左右分屏。移动端 Rust target 暂时使用同名播放器命令返回明确 unsupported 状态，不链接桌面 `libmpv-sys`，因此该 APK 是 UI、数据源和窗口适配预览，不是可播放版本。

这些工作不代表 Android 已可发布。Android 版本仍需独立完成 ARM64 libmpv `.so`、`SurfaceView` / `TextureView` 原生渲染、媒体与目录权限、系统返回键、系统栏避让和移动端更新策略；完成原生播放器后再移除 mobile player stub。桌面窗口拖拽、最小化、最大化与关闭按钮在移动环境中不得作为导航依赖。
