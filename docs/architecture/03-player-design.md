...
单视频偏好恢复分为两段：倍速、字幕偏移和画面模式可立即恢复；字幕、音轨和缓存外部字幕必须等待时长/轨道元数据出现后再匹配。`sid`、`aid` 与 `sub-add` 进入同一个有序 libmpv 异步命令队列，Rust 立即释放 `MpvState` 锁；事件转发器读取 `MPV_EVENT_COMMAND_REPLY`，把真实成功、失败或超时结果返回调用方后才执行下一条轨道命令。禁止只把“命令已排队”当成字幕已加载，也禁止丢弃异步命令错误。用户手动选择字幕/音轨或下载字幕时取消尚未完成的旧轨道偏好恢复，避免旧选择覆盖新操作。Windows 缓存字幕送入 mpv 前移除 `\\?\` 扩展路径前缀，同时保持 SQLite 中的规范化受控路径不变。
