...
OpenList/Alist、CloudDrive2、本地文件等原始文件源在完成本地扫描后，数据源首页应与 Emby/Jellyfin 保持同一用户心智：顶部是大海报/背景轮播，下面是媒体库卡片，再进入具体分类的作品海报墙。标准目录优先使用明确的路径分类目录作为媒体库分类，例如 `动漫`、`综艺`、`国产剧`；非标准目录或没有清晰路径分类时，再使用 TMDB 元数据和本地分类规则生成分类。分类不应来自作品目录名或发布组噪声目录名。
