...
下表描述目标平台方案。Windows MVP 渲染路径已完成宿主实机验证；Android ARM64 已完成 `SurfaceView` + 官方 mpv-android runtime 的构建与 APK 包内契约验证，但尚无连接设备完成真机画面、硬解与生命周期验证。macOS、Linux 渲染后端仍未完成，不作为当前 CI/release 阻塞项。
