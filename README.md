...
- **OpenList/Alist 与 CloudDrive2 数据源**：账号密码登录、从 `/` 浏览并选择只读根目录、目录浏览、搜索和云盘文件播放；CloudDrive2 使用 WebDAV + Basic Auth，播放凭据通过临时 header 传入播放层。
- **本地只读刮削与海报墙**：OpenList/Alist、CloudDrive2 和本地文件夹选中根目录后可在 Player 本地扫描，生成媒体库分类、作品海报墙、未识别兜底和扫描日志，不重命名、不移动、不写回网盘。
