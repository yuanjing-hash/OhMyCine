...
- Media-detail still and people rows use the shared immersive horizontal rail: native operating-system scrollbars stay hidden, desktop gets keyboard-accessible glass previous/next controls plus edge fades, and touch/coarse-pointer layouts retain direct inertial scrolling without hover-only controls. Stills use large 16:9 artwork cards and cast uses portrait cards; adaptive playback options and technical metadata must fill available width instead of leaving large empty glass panels.
