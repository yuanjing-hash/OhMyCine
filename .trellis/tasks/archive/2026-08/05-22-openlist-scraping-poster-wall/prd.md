...
* TMDB 是第一优先级外部元数据源，但用户 TMDB API Key / Read Access Token 只是可选增强；未配置凭据时扫描不能失败，必须保留可播放候选、目录识别结果和兜底分类。内置/公共元数据通道作为后续设计，不在客户端硬编码第三方密钥。
...
用户第一次进入媒体库视图时，如果该数据源尚未扫描，页面显示“开始扫描”主操作、扫描范围摘要（当前 `rootPath`）和可选 TMDB 配置状态。扫描完成后，默认进入海报墙；未识别文件可以从“未识别”或文件夹视图继续访问和播放。
...
* 默认呈现与 Emby 数据源一致的媒体库首页：顶部大海报/背景轮播，下面是自动扫描和 TMDB 分类规则生成的逻辑媒体库卡片。
* 媒体库卡片不是物理目录卡片；点击后进入该分类的作品海报墙，电影和剧集都按作品聚合展示。
* 扫描状态、重新扫描、结构判断和日志放进显式“扫描管理”入口，不占据默认浏览页面。
* 文件夹视图作为兜底入口，通过页面操作按钮进入，不作为默认主视图。
* 文件夹视图继续走当前 DataSource `list()`，不依赖刮削成功。
* 海报卡点击进入与 Emby 同形态的详情页；电影直接播放主文件，剧集进入季/集列表后播放具体集。

设置页：

* 保持“管理数据源”作为设置子页。
* 增加“刮削与分类”设置项，管理可选 TMDB 凭据、默认语言地区、默认扫描模式、分类规则。
...
3. TMDB 与分类：可选 TMDB 配置、搜索/详情、海报缓存、默认分类规则和规则执行；无凭据时走本地候选和兜底分类。
4. UI 接入：OpenList/Alist 媒体库视图、海报墙、分类筛选、详情页剧集列表、扫描日志入口。

## Acceptance Criteria (evolving)

* [ ] 用户能在 OpenList/Alist 数据源上手动触发扫描。
* [ ] 未配置 TMDB 凭据时扫描仍能完成本地候选解析，媒体库不空白，文件夹浏览和播放保持可用。
