...
- [ ] 建立跨 Go/TypeScript/SDK 的契约夹具，防止字段漂移。
...
- [ ] 增加插件包、安装、运行代次、权限授权、连接、在线库和私有状态模型及显式迁移。
- [ ] 实现隔离插件目录、旁路解包、路径穿越防护、摘要/签名和原子切换。
- [ ] 集成 WASM Host，限制内存、调用时间、并发和 Host capability。
- [ ] 实现受控 HTTP、凭据句柄、私有 KV、结构化日志与审计。
- [x] 实现安装、启停、升级权限差异、失败回滚和卸载服务/API。
- [x] 增加插件 RBAC 权限目录。
- [ ] 完成 Server 插件管理页：卡片、健康、版本、权限、连接、日志入口和操作确认。
- [ ] 增加“已安装 / 插件市场 / 仓库设置”分页；仓库设置支持添加、停用、排序、删除和手动刷新 GitHub 仓库地址，市场页只展示通过 Registry/Manifest 校验的插件。
...
- [ ] 实现声明式 Feed、cursor、refresh session、缓存和限速。
- [x] 扩展 `/api/v1/player/*`，只返回安全 opaque ID 和 DTO。
- [ ] 实现 HomeContribution 候选和来源级错误。
- [ ] 实现站点动作权限校验和幂等请求标识。
...
- [ ] 扩展 Server 媒体条目版本描述，并从现有 release version parser 保守回填。
- [ ] 将剧集响应整理成 Work → Segment → Version，修复同一集多个版本显示为重复集数的问题。
- [ ] 扩展 Player 类型和运行时校验，兼容旧 Server。
- [ ] 扩展 MediaStreamRequest/PlaybackPlan 支持 DASH、variants、期限和刷新选择令牌。
- [ ] 统一电影、剧集、Bilibili 分 P 的内容版本模型，以及与版本分离的 StreamVariant 清晰度模型。
- [ ] 抽取并复用现有播放队列/选集菜单，只负责集数/分 P 和媒体版本选择。
- [ ] 在 PlayerControls 增加独立清晰度按钮，只切换当前版本的 StreamVariant；少于两个可用档位时隐藏。
- [ ] 实现播放中无损切换：先解析、成功替换、恢复进度/状态、失败回退。
- [ ] 将播放进度、版本偏好和字幕/音轨偏好分别绑定正确身份层级。
- [ ] 验证多版本入库、冲突、NFO 和 STRM 不互相覆盖。
...
- [ ] 实现声明式站点主页、导航、栏目、搜索和详情渲染。
- [ ] 实现来源徽标、站点动作和不支持能力的隐藏策略。
- [ ] 实现设备侧主页来源/Feed 启用、顺序和位置配置。
- [ ] 顶部 Hero 混合可信来源，下方按来源分栏。
- [ ] 实现全局刷新和单栏目刷新，保持旧推荐会话滚动状态。
...
- [ ] 实现扫码登录、二维码过期、轮询限速、会话加密保存和重新认证。
- [ ] 实现主页推荐刷新、热门、排行、专区和分页。
- [ ] 实现搜索、详情、UP 主、合集、分 P/剧集。
- [ ] 实现收藏、稍后再看、历史、关注与追更读取。
- [ ] 按能力逐项实现远端写操作并提供明确确认/错误。
- [ ] 实现账号权限内的播放清晰度、DASH、字幕和弹幕解析。
...
- [ ] 定义并实现宿主 DownloadPlan 执行器，不允许插件提供任意命令或路径。
- [ ] 增加 FFmpeg MediaTool 抽象、版本探测、固定参数和安全日志。
- [ ] 增加 Windows 隔离工具安装脚本，固定版本、SHA-256、许可证并 gitignore 工具目录。
- [ ] 下载视频、音频、字幕/弹幕到任务专属暂存目录。
- [ ] 持久化解析、视频下载、音频下载、合流、校验和入库阶段。
- [ ] 实现暂停/取消/重试及任务受管临时产物清理。
- [ ] 合流完成后生成标准 manifest，进入现有识别、整理、转移和历史链路。
- [ ] 保留清晰度/版本后缀，验证多版本不被冲突策略误覆盖。
...
- [ ] 发布 Manifest/DTO Schema、SDK、fixture、模板插件和 mock host。
...
- [ ] 更新 Hub 插件详情、权限、兼容矩阵、校验状态和升级差异展示。
...
- [ ] 最终审查 PT 内建边界未被破坏，现有本地/115/Emby 数据源无回归。
