...
const androidNcnnRuntime = await source('src-tauri/gen/android/app/src/main/cpp/rife_ncnn_runtime.cpp')
...
assert.match(androidNcnnRuntime, /network\.load_param/)
assert.match(androidNcnnRuntime, /network\.load_model/)
assert.match(androidNcnnRuntime, /use_vulkan_compute = true/)
assert.match(androidNcnnProbe, /RifeNcnnCompatibilityMode::SafeFp32/)
assert.match(androidNcnnProbe, /RifeNcnnCompatibilityMode::SafeFp32HostWeights/)
assert.match(androidNcnnRuntime, /use_subgroup_ops = false/)
assert.match(androidNcnnRuntime, /use_cooperative_matrix = false/)
assert.match(androidNcnnRuntime, /param_status = network\.load_param/)
assert.match(androidNcnnRuntime, /model_status = network\.load_model/)
...
assert.match(kotlinHost, /MpvEvent\.VIDEO_RECONFIG[\s\S]*?onMediaEvent\(FrameInterpolationMediaEvent\.VIDEO_RECONFIG\)[\s\S]*?requestFrameInterpolationReconcile\(\)/)
assert.match(kotlinHost, /MpvEvent\.PLAYBACK_RESTART[\s\S]*?requestFrameInterpolationReconcile\(\)/)
assert.match(kotlinHost, /frameInterpolationSurfaceTransition[\s\S]*?video-reconfig-surface-transition/)
assert.match(kotlinHost, /beginFrameInterpolationSurfaceTransition\(\)[\s\S]*?frameInterpolationSurfaceTransitionToken/)
...
assert.match(windowsDirectMlProbe, /consecutive_generated_pairs >= 2/)
assert.match(windowsDirectMlProbe, /stalled_ms > 350\.0[\s\S]*?hide_generated_output/)
assert.match(windowsDirectMlProbe, /SystemRelativeTime[\s\S]*?capture_seconds[\s\S]*?anchor_seconds/)
assert.match(windowsDirectMlProbe, /run_inference_watchdog[\s\S]*?elapsed_ms > 250\.0[\s\S]*?SetTerminate/)
assert.match(windowsDirectMlProbe, /dropped_output_ticks\.fetch_add/)
assert.match(windowsDirectMlProbe, /latest_inference_micros\.store/)
assert.match(windowsDirectMlProbe, /bool flow_ready = false[\s\S]*?interpolated && !flow_ready[\s\S]*?flow_ready = true/)
assert.match(windowsDirectMlProbe, /TimestepConstants timestep_params\{\s*0\.5f/)
assert.match(windowsDirectMlProbe, /flowTimestep[\s\S]*?earlierFlow[\s\S]*?laterFlow/)
assert.match(windowsDirectMlProbe, /selector == 48[\s\S]*?ProxyExtent\{64, 32\}[\s\S]*?ProxyExtent\{32, 64\}/)
assert.doesNotMatch(windowsDirectMlProbe, /ProxyExtent\{48, 48\}/)
assert.match(windowsDirectMlProbe, /image_shape\{1, 3, proxy\.height, proxy\.width\}/)
assert.match(windowsDirectMlProbe, /CreateWindowExW\([\s\S]*?state->source_hwnd/)
assert.match(windowsAssetGate, /cadence_stalled/)
assert.match(windowsAssetGate, /flow_scale <= 0\.5[\s\S]*?32[\s\S]*?flow_scale <= 0\.67[\s\S]*?48[\s\S]*?64/)
assert.match(rustPlayer, /status\.cadence_stalled[\s\S]*?restore_windows_frame_interpolation_audio_delay/)
assert.match(rustPlayer, /restore_windows_frame_interpolation_audio_delay[\s\S]*?set_property\("audio-delay"[\s\S]*?is_ok\(\)[\s\S]*?audio_delay_original = None/)
assert.match(rustPlayer, /record_model_time\([\s\S]*?status\.latest_inference_ms/)
assert.match(rustPlayer, /record_drops\(new_drops\)/)
